...
public:
	...
	// 인접 행렬을 출력하는 함수
	template <typename T>
	friend ostream& operator <<(ostream& o, const ADjacencyMatrix<T>& matrix);
	~ADjacencyMatrix()
	{
		if (matrix != nullptr)
		{
...
template <typename T>
ostream& operator << (ostream& o, const ADjacencyMatrix<T>& matrix)
{
	for (int i = 0; i < matrix.matrixCount; i++)
	{
		for (int j = 0; j < matrix.matrixCount; j++)
		{
			o << matrix.matrix[i][j] << " ";
		}
		o << endl;
